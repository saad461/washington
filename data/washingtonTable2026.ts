 ...
 * Handles all legal rules, strict validation, and deterministic table lookups.
 *
 * @param income - Combined Monthly Net Income
 * @param children - Number of Children in case (Strict integer 1-5)
...
export function getExactSupport(income: number, children: number, debug: boolean = false): SupportCalculationResult {
  ...
  if (children === null || children === undefined || isNaN(children)) {
    return { status: "INVALID_INPUT", reason: "Children input is missing or invalid", debug: debugInfo };
  }

  // 2. Strict Children Logic: Must be integer 1-5. No silent clamping or rounding.
  ...
  // 3. Legal Rule: Combined Net Monthly Income < 2200 requires manual determination
  if (income < 2200) {
    return {
  ...
  // 4. Income Handling: Max usable income = 50000 (clamp for lookup)
