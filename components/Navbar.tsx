...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="container-wide h-full flex items-center justify-between gap-4">
        ...
        >
          ...
          <div className="flex flex-col leading-none font-heading">
            ...
            {/* FIX: was text-[9px] — unreadable. Now text-[10px] minimum. */}
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {/* FIX: removed !important overrides — btn-primary used cleanly */}
      ...
      {/* ── Mobile Drawer ──────────────────────────────────────────────────
          FIX: top offset matches nav height via dynamic class (no mismatch).
          FIX: overflow-y-auto so tall content doesn't get clipped.
          FIX: backdrop dims page content, clicking it closes the drawer.
      ──────────────────────────────────────────────────────────────────── */}
      {mobileOpen && (
        <>
          <div
            className={`lg:hidden fixed inset-x-0 bottom-0 bg-black/20 backdrop-blur-[2px] z-40 ${drawerTop}`}
          ...
          <nav
            ...
            className={`lg:hidden fixed inset-x-0 bottom-0 bg-white z-50 overflow-y-auto
              flex flex-col animate-in fade-in slide-in-from-top-2 duration-200 ${drawerTop}`}
          >
            {/* Links */}
