      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5 group transition-transform active:scale-95">
          ...
          <div className="flex flex-col text-left">
            ...
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none mt-1">WA Schedule Center</span>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-8 translate-x-4">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-[10px] font-black text-slate-400 hover:text-indigo-600 transition-colors uppercase tracking-[0.2em] py-1 border-b-2 border-transparent hover:border-indigo-600"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={handlePrint}
            className="hidden sm:flex items-center gap-2 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-all border border-transparent hover:border-slate-200"
          >
            <Printer size={16} />
          ...
          <Link 
            href="/worksheet" 
            className="hidden sm:flex group items-center justify-center bg-indigo-600 shadow-xl shadow-indigo-500/20 text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all active:scale-95"
          >
            Launch Wizard
          ...
          <nav className="flex flex-col px-6 py-6 space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                className="flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 transition-all"
              >
                {link.label}
              </Link>
            ))}
            <div className="border-t border-slate-100 pt-4 mt-2 space-y-1">
              <Link href="/terms" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold text-slate-400 hover:bg-slate-50 hover:text-slate-700 transition-all">
                Terms of Service
              </Link>
              <Link href="/disclaimer" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-bold text-slate-400 hover:bg-slate-50 hover:text-slate-700 transition-all">
                Legal Disclaimer
              </Link>
            </div>
            <div className="pt-4">
