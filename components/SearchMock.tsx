    ...
    <div className={`w-full ${isSidebar ? '' : 'max-w-xl mx-auto'} no-print`}>
      <div className="relative group">
        ...
        <input
          ...
          className={`w-full ${isSidebar ? 'py-4 px-14 text-sm' : 'py-6 px-16 text-lg'} bg-white border-2 border-gray-100 rounded-2xl shadow-xl shadow-indigo-900/5 focus:border-indigo-600 focus:ring-0 transition-all outline-none font-medium placeholder:text-gray-300`}
        />
      </div>

      {query.length > 0 && (
        <div className="absolute z-50 mt-4 w-full bg-white border border-gray-100 rounded-2xl shadow-2xl p-4 overflow-hidden animate-in fade-in slide-in-from-top-2">
          {filtered.length > 0 ? (
            <div className="space-y-1">
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-gray-500 uppercase tracking-[0.2em]">{county.seat}, WA</p>
                    </div>
                  </div>
              ...
              <div className="pt-4 px-4 border-t border-gray-50 mt-2 text-center">
                <p className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.2em]">Showing Top Results for {query}</p>
              </div>
            </div>
          ) : (
